...
export default function ITReturnsFiling() {
  ...
  function validateStep() {
    switch (step) {
      ...
      case 2:
        ...
        break;
      case 3:
        if (!formData.deductions.trim()) {
          alert("Please enter deductions or type 'None'.");
          return false;
        }
        break;
      case 4:
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden font-sans">
      ...
      <div
        ...
        style={{ backgroundImage: "url('/itr-filing-bg.jpg')" }}
      >
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 3 && (
              ...
              >
                ...
                <textarea
                  rows={4}
                  placeholder="Deductions (e.g. 80C, 80D, other tax-saving instruments)"
                  value={formData.deductions}
                  onChange={(e) => updateField("deductions", e.target.value)}
                  className="border border-gray-300 p-4 rounded-xl w-full text-lg resize-none focus:outline-none focus:ring-2 focus:ring-indigo-600"
                />
              </motion.div>
            )}
            ...
            {step === 5 && (
              ...
              >
                ...
                <div className="border border-gray-200 rounded-xl p-6 bg-gray-50 shadow-sm max-h-[340px] overflow-auto text-left space-y-6">
                  ...
                  <div>
                    <h3 className="text-indigo-800 font-semibold mb-1">PAN Number</h3>
                    <p className="text-gray-700">{formData.pan || "-"}</p>
                  </div>
                  <div>
                    <h3 className="text-indigo-800 font-semibold mb-1">Total Income</h3>
                    <p className="text-gray-700">
                      {formData.income ? `₹${Number(formData.income).toLocaleString()}` : "-"}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-indigo-800 font-semibold mb-1">Deductions</h3>
                    <p className="whitespace-pre-wrap text-gray-700">{formData.deductions || "-"}</p>
                  </div>
                  <div>
